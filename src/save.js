  ...
  return (
    <div className='w-full min-h-screen flex items-center justify-center'>
     ...
     {fields && (
        ...
        animate={{ opacity: 1}}
        exit={{ opacity: 0}}
        className={'w-full p-2 rounded-lg font-semibold ${ alertStatus === "danger" ? "bg-red-400 text-red-800" : "bg-emerald-400 text-emerald-800" }'}
        >
          {msg}
          </motion.p>
      )}
      ...
      <div className='w-full'>
        <select onChange={(e) => setCategory(e.target.value)} className="outline-none w-full text-base order-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
      ...
      <div className='group flex justify-center items-center flex-col border-2 border-dotted border-gray-300 w-full h-225 md:h-420 cursor-pointer rounded-lg'>
        ...
        {!imageAsset ? ( 
        <>
        <label className="w-full h-full flex fle-col items-center justify-center cursor-pointer">
        ...
        {/* <div className='relative h-full'>
          ...
          <button type='button' className='absolute bottom-3 right-3 p-3 rounded-full bg-red-500 text-xl cursor-pointeroutline-none hover:shadow-md duration-500 transition-all ease-in-out'
          onClick={deleteImage}
